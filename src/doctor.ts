// `opencontrol doctor --gamepad` — the legacy standalone controller diagnostic. No agent is wrapped
...
import { atomicWritePrivateFile } from './secure-files.js'
...
export interface RunGamepadDoctorOptions {
  reportPath?: string
  overwrite?: boolean
}
 ...
 */
export async function runDoctor(options: RunGamepadDoctorOptions = {}): Promise<void> {
  ...
  try {
    ...
    if (!driver) {
      ...
      if (!candidate?.path) {
        console.error(
          'No controller detected. Plug one in and rerun `opencontrol doctor --gamepad`.',
        )
        return
      }
      ...
      const report = buildReport(
        {
          ...
          product: sanitizeHidProduct(candidate.product),
      ...
      writeGamepadDoctorReport(report, options)
      return
    }
    ...
    const product = sanitizeHidProduct(device?.product)
    console.log('\nController detected:')
    console.log(
...
function buildReport(
  ...
  return {
    ...
    controller: { ...controller, product: sanitizeHidProduct(controller.product) },
    results,
    axes,
    output,
...
/** Securely write the report/fixture and print the markdown summary + PR hint. */
export function writeGamepadDoctorReport(
  report: DoctorReport,
  options: RunGamepadDoctorOptions = {},
): void {
  const safeReport: DoctorReport = {
    ...report,
    controller: {
      ...report.controller,
      product: sanitizeHidProduct(report.controller.product),
    },
  }
  const name = fixtureName(
    safeReport.controller.vid,
    safeReport.controller.pid,
    safeReport.controller.transport,
  )
  const reportPath = options.reportPath ?? join(process.cwd(), name)
  atomicWritePrivateFile(reportPath, JSON.stringify(safeReport, null, 2) + '\n', {
    overwrite: options.overwrite ?? false,
  })
  console.log(`\nWrote ${sanitizeTerminalText(reportPath, 240)}`)
  console.log(
    `To contribute: drop ${name} into test/fixtures/controllers/ and open a PR — CI replays its captures automatically.\n`,
  )
  console.log('```markdown')
  console.log(markdownSummary(safeReport))
  console.log('```')
}

function sanitizeHidProduct(value: string | undefined): string {
  const sanitized = sanitizeTerminalText(value ?? '', 80)
    .replace(/\s+/gu, ' ')
    .trim()
