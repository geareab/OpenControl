...
import { Dualsense, DualsenseHID, NodeHIDProvider } from 'dualsense-ts'
...
export class DualSenseDriver extends EventEmitter implements ControllerHAL {
  ...
  private provider: NodeHIDProvider | null = null
  ...
  start(): void {
    try {
      const provider = new NodeHIDProvider({ devicePath: this.devicePath })
      const c = (this.controller = new Dualsense({
        hid: new DualsenseHID(provider),
      }))
      ...
      void provider
        .connect()
        .then(() => {
          // A slow dynamic import/open must not resurrect a driver stopped in
          // the meantime.
          if (this.provider !== provider) provider.disconnect()
        })
        .catch((error) => logger.error('DualSenseDriver connection failed', error))
    } catch (err) {
