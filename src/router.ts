...
export class LayerRouter {
  ...
  /** Clear physical input state when a controller disconnects or is replaced. */
  resetInputState(): void {
    this.held.clear()
    this.ignoredHeld.clear()
    this.l1Held = false
  ...
  private routeButton(button: ButtonId, pressed: boolean): Action | null {
    ...
    if (pressed) {
      if (this.held.has(button)) return null
  ...
  private routeAxis(axis: AxisId, value: number): Action | null {
    ...
    const action = this.lookup(gesture)
    if (isRotaryGesture(gesture) && action && !isRepeatableAction(action)) return null
    return action
  }
}

function isRotaryGesture(controlId: ControlId): boolean {
  return (
    controlId === 'lstick_cw' ||
    controlId === 'lstick_ccw' ||
    controlId === 'rstick_cw' ||
    controlId === 'rstick_ccw'
  )
}
