...
function reportUnsupported(harness: Harness, action: Action): void {
  const key = `${harness.kind}:${JSON.stringify(action)}`
  if (warnedUnsupported.has(key)) return
  warnedUnsupported.add(key)
  logger.warn(`Action is not supported by ${harness.kind}`, action)
}
...
export interface DispatchDeps {
  ...
  /** Move selection by one occupied task slot. */
...
export function dispatchAction(action: Action, deps: DispatchDeps): void {
  switch (action.type) {
    case 'focus_session':
      ...
      return
    case 'focus_relative':
      deps.focusRelative?.(action.delta)
      return
    case 'layer':
    ...
    default: {
      ...
      if (!resolved) {
        reportUnsupported(deps.harness, action)
        return
      }
