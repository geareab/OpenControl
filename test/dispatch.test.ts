...
function makeDeps(overrides: Partial<DispatchDeps> = {}): {
  ...
  const deps: DispatchDeps = {
    ...
    focusRelative: (delta) => focus.push(delta === 1 ? 100 : -100),
    setLayer: (i) => layers.push(i),
    ...overrides,
  }
  return { deps, writes, focus, layers, levels }
}
...
describe('dispatchAction', () => {
  ...
  it('routes relative task selection through core', () => {
    const { deps, focus, writes } = makeDeps()
    dispatchAction({ type: 'focus_relative', delta: 1 }, deps)
    dispatchAction({ type: 'focus_relative', delta: -1 }, deps)
    expect(focus).toEqual([100, -100])
    expect(writes).toEqual([])
  })
