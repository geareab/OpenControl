...
describe('claude harness', () => {
  ...
  it('maps hook events to states', () => {
    ...
    expect(claudeHarness.stateForHookEvent('StopFailure', {})).toBe('error')
  ...
  it('resolves the verified keybindings', () => {
    ...
    expect(claudeHarness.resolveAction({ type: 'push_to_talk' }, ctx)).toBeNull()
  ...
  it('maps portable command actions', () => {
    expect(claudeHarness.resolveAction({ type: 'agent_command', command: 'fast' }, ctx)).toEqual({
      bytes: '/fast\r',
    })
    expect(claudeHarness.resolveAction({ type: 'agent_command', command: 'fork' }, ctx)).toEqual({
      bytes: '/branch\r',
    })
    expect(claudeHarness.resolveAction({ type: 'agent_command', command: 'skills' }, ctx)).toEqual({
      bytes: '/skills\r',
    })
    expect(claudeHarness.resolveAction({ type: 'agent_command', command: 'mic' }, ctx)).toBeNull()
  })
...
describe('codex harness', () => {
  ...
  it('maps portable command actions without guessing unavailable features', () => {
    expect(codexHarness.resolveAction({ type: 'agent_command', command: 'fast' }, ctx)).toEqual({
      bytes: '/fast\r',
    })
    expect(codexHarness.resolveAction({ type: 'agent_command', command: 'fork' }, ctx)).toEqual({
      bytes: '/fork\r',
    })
    expect(
      codexHarness.resolveAction({ type: 'agent_command', command: 'model_picker' }, ctx),
    ).toEqual({ bytes: '/model\r' })
    expect(codexHarness.resolveAction({ type: 'agent_command', command: 'mic' }, ctx)).toBeNull()
  })
})
